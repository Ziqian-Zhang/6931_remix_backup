...
// File: contracts/DAOGTokenTest.sol
...
contract DAOGToken is ERC20, Ownable {
   ...
   function calculateVestingBonus(uint256 amount, uint256 duration) internal pure returns (uint256) {
       uint256 months = duration / 1 minutes; // 1 minute = 1 "month" for fast testing
       return (amount * months) / 100; // 1% bonus per simulated month
   }
}
